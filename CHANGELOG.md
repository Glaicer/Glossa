...
- **Desktop Secret Service integration.** API keys are no longer stored as plaintext in `config.toml`. Glossa now supports a `secret-service:{slot}` syntax for provider and LLM API keys, backed by the standard Secret Service (via `libsecret`/`org.freedesktop.secrets`).
  - New CLI command: `glossa store-secret {provider,llm}` reads a key from stdin and stores it securely.
  - New CLI command: `glossa migrate-secrets` converts existing plaintext keys in `config.toml` to Secret Service entries.
  - The daemon automatically migrates plaintext keys on startup and after every restart.
  - The interactive installer now stores the provider API key in Secret Service automatically instead of writing it into `config.toml`.
  - The tray `Settings` dialog now stores API keys in Secret Service instead of writing them back into `config.toml`.
- Recordings now automatically stop after the configured `audio.max_duration_sec` to prevent unintentionally long captures.

### Changed

- The example config generated by the installer now defaults to `max_duration_sec = 1200` (was 120).
- Default `api_key` in generated and example configs is now `secret-service:provider` instead of `env:GROQ_API_KEY`.
